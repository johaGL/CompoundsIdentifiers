...
interesting_columns = ['lm_id', 'kegg_id']
final_column_names = ['LipidMaps', 'KEGG']
the_dfs_dict = {}
...
for u in lipidmaps_groups:
    print(f"\nretrieving {u}\n")
    response = requests.get(f"https://www.lipidmaps.org/rest/compound/lm_id/{u}/all/download")
    txt = response.text
    lines = txt.split("\n")

    download_date = lines[0].replace("/", "-")  # the first line is a download_date
    print(download_date)
    ...
    df = pd.DataFrame.from_dict(foo).T  # TODO: recheck this in new panda sversion

    df.columns = header
    df = df[interesting_columns]
    df = df.dropna()
    df = df.loc[df['kegg_id'] != "", :]
    the_dfs_dict[u] = df
# end for
...
final_df = pd.concat(dfs_list_to_concat, axis=0)  # TODO re-do with new pandas version to verify axis
final_df.columns = final_column_names
final_df.to_csv(f"../tables/LipidMaps-to-KEGG_{download_date}.tsv", sep='\t', index=False)
